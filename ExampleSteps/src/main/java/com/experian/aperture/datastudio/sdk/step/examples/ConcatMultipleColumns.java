...
public class ConcatMultipleColumns extends StepConfiguration {
                ...
                .withArgTextSupplier(sp -> () -> {
                    ...
                    if (sp.allowedValuesProvider == null) {
                        ...
                        if (value != null) {
                            ...
                            try {
                                final JSONArray columnNameArray = new JSONArray(value.toString());
    ...
    private class ConcatValuesOutput extends StepOutput {
        ...
        public Object getValueAt(final long row, final int col) throws SDKException {
            ...
            final String selectedColumns = getArgument(0);
            try {
                final JSONArray columnNameArray = new JSONArray(selectedColumns);

                // get the value for each user-defined column and concatenate them with spaces
                for (int i = 0; i < columnNameArray.length(); i++) {
                    final String columnName = columnNameArray.get(i).toString();
                    final Optional<StepColumn> column = getInputColumn(0, columnName);
                    if (column.isPresent()) {
                        value.append(" ");
                        value.append(column.get().getValue(row));
                    }
                }
